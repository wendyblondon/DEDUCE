...
*Software:* DEDUCE app version 1.0 available [here](https://deduce.shinyapps.io/DEDUCE/) 
...
Trial operating characteristics are averaged over **`r params$m[1]`** simulated trials. Simulated trials have **`r params$m[2]`** dose levels labeled **`r dQuote(params$m[3])`**, starting on dose level **`r dQuote(params$m[4])`**, and assuming true toxicity probabilities of **`r params$m[5]`**. The target toxicity probability is **`r params$m[6]`**. One patient arrives every **`r params$m[7]`** days on average. The DLT observation period is **`r params$m[8]`** days. The seed is **`r params$m[14]`**. 
