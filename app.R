...
server <- function(input, output, session) {
  ...
  DTPlot1 <- reactive({
    if(DTSelectedDesignsLength() > 1){
        ...
        ggplot() + 
        geom_bar(data=DTPlotDF() %>% 
                   mutate(MTD.Prop = MTD.Freq/input$DTNumTrials), aes(x=DoseLevel, y=MTD.Prop), stat='identity', color="#E8E8E8") + 
        geom_bar(data=DTPlotDF() %>% 
  ...
  DTPlot2 <- reactive({
    if (DTSelectedDesignsLength() > 1){
      ...
      ggplot() + 
      geom_bar(data = DTPlotDF(), aes(x=DoseLevel, y=obs.tox.table), stat="identity", position="dodge", color="#E8E8E8") + 
      geom_bar(data = DTPlotDF() %>% 
  ...
  DTPlot3 <- reactive({
    if (DTSelectedDesignsLength() > 1){
      ...
      ggplot() + 
        geom_bar(data=DTPlotDF(), aes(x=DoseLevel, y=patient.allocation.table), stat="identity", position="dodge", color="#E8E8E8") +
        geom_bar(data=DTPlotDF() %>% 
